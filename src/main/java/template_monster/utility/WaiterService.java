...
public class WaiterService {
    ...
    public static void waitPageLoader(String url, WebDriver driver){
        ...
        int attempt_counter = 0;
        while (!ManageUrlService.validateStringInUrl(url, driver)){
            WaiterService.sleep(1);
            attempt_counter++;
        ...
        ReportService.assertTrue(ManageUrlService.validateStringInUrl(url, driver), "\"" + url + "\" page was not loaded after 30 seconds.");

    }

    public static void waitElementBorderColor(WebElement element,String elementName, String color){
        int attempt_counter = 0;
        while (!WebElementService.getElementBorderColor(element, elementName).equals(color)){
    ...
    public static void waitForElementDisappear(WebElement element, String elementName){
        ...
        int attempt_counter = 0;
        while (WebElementService.elementIsDisplayed(element,elementName)) {
           WaiterService.sleep(1);
           attempt_counter++;
           if (attempt_counter == 5){
